Write-Host "=== Superset Post-Provision Hook ==="
...
Write-Host "1. Getting AKS credentials..."
az aks get-credentials --resource-group $RESOURCE_GROUP --name $AKS_CLUSTER --overwrite-existing

Write-Host "2. Installing NGINX Ingress Controller..."
kubectl apply -f https://raw.githubusercontent.com/kubernetes/ingress-nginx/controller-v1.8.2/deploy/static/provider/cloud/deploy.yaml

Write-Host "3. Waiting for NGINX Ingress Controller to be ready..."
kubectl wait --namespace ingress-nginx --for=condition=ready pod --selector=app.kubernetes.io/component=controller --timeout=300s 2>$null

# Wait for external IP
Write-Host "4. Waiting for external IP assignment..."
$EXTERNAL_IP = ""
for ($i = 1; $i -le 60; $i++) {
    ...
    Write-Host "   Attempt $i/60: Waiting for IP..."
    Start-Sleep -Seconds 10
}

if (-not $EXTERNAL_IP) {
    Write-Host "ERROR: Failed to get external IP after 10 minutes"
    exit 1
}

Write-Host "5. Creating superset namespace..."
kubectl create namespace superset --dry-run=client -o yaml | kubectl apply -f -

Write-Host "6. Creating ConfigMap for superset_config.py..."
@"
apiVersion: v1
kind: ConfigMap
metadata:
...
Write-Host "7. Creating Kubernetes Secret..."
...
Write-Host "8. Deploying Superset Deployment + Service + Ingress..."
@"
apiVersion: apps/v1
kind: Deployment
metadata:
...
Write-Host "9. Waiting for Superset pod to be ready (this may take 3-5 minutes)..."
kubectl wait --namespace superset --for=condition=ready pod --selector=app=superset --timeout=600s 2>$null

Write-Host "10. Checking pod status..."
...
Write-Host ""
Write-Host "=== Superset Deployment Complete ==="
