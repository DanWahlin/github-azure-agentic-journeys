...
- Invoke child processes with argument arrays and don't build interpolated shell strings. On macOS and Linux, execute `az` and `azd` directly. On Windows, use the repository's static `powershell.exe` JSON-payload launcher so Node does not try to execute Azure CLI `.cmd` shims directly. Because `.cmd`/`.bat` cannot losslessly represent shell metacharacters or CR/LF inside arguments, the launcher fails closed for those cases; rewrite the argument, attach a script for complex payloads, or invoke a native executable target.
