...
Run `node ../../.github/scripts/verify-superset.mjs` from `journeys/superset`; it fails if the pod isn't Ready/Running, PostgreSQL evidence is absent, SQLite appears in logs, or `/health` doesn't return HTTP 200. To isolate the package import, pass `kubectl exec -n superset <pod> -c superset -- python -c "import psycopg2; print('OK')"` to `az aks command invoke`. Do not invoke a local `kubectl` binary.
